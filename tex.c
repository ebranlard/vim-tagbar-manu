...
#include <stdio.h>	//MANU
#include <string.h>	//MANU
...
static void makeTexTag (tokenInfo *const token, texKind kind)
{
	if (TexKinds [kind].enabled)
	{
		...
		// MANU For "Input" kind, we change the source file to the target file. For that we use the tag name and replace space with "/"
// 		printf("kind %s\n", &e.kind);
		if (kind == TEXTAG_INPUT){
// 			printf("YES\n");
// 			printf("%s\n", (&e)->sourceFileName);
// 			printf("%s\n", (&e)->name);
			// We change the source file using the tag. But the tag has spaces instead of slashes...
			(&e)->sourceFileName=(&e)->name;
			mystring=(char *)malloc(sizeof(char)*strlen((&e)->name));
			count=0;
			count2=0;
				while ( count < strlen((&e)->name)){
					if ((&e)->name[count] == ' ')
					{
						mystring[count2]='/';
						count++;
						...
						mystring[count2]=(&e)->name[count];
					}
					count++;
					count2++;
				}
			(&e)->sourceFileName=mystring;
// 			printf("srcf %s\n", (&e)->sourceFileName);
// 			printf("name %s\n", (&e)->name);
// 			printf("myst %s\n", mystring);
// // 			(&e)->lineNumberEntry=1;
		...
		if (isType (token, TOKEN_KEYWORD))
		{
			switch (token->keyword)
			{
				...
				case KEYWORD_include:
					...
					break;
				case KEYWORD_input: //MANU
					parseTag (token, TEXTAG_INPUT);
					break;
				default:
					break;
			}
